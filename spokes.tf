  ...
  routes = [
    {
      ...
      next_hop_ilb      = module.lb_internal_hub1.address
    }
  ]
  ...
  routes = [
    {
      ...
      description       = "Default route to VM-Series NGFW"
      destination_range = "0.0.0.0/0"
      next_hop_ilb      = module.lb_internal_hub2.address
    }
  ]
...
# -------------------------------------------------------------------------------------
# Create VPC peering connections between the hub1 & spoke1 and hub2 & spoke2.
# -------------------------------------------------------------------------------------
...
module "peering_spoke1" {
  ...
  count                                     = (var.create_spoke_networks ? 1 : 0)
  version                                   = "~> 5.2.0"
  local_network                             = module.vpc_hub1.network_id
...
module "peering_spoke2" {
  ...
  count                                     = (var.create_spoke_networks ? 1 : 0)
  version                                   = "~> 5.2.0"
  local_network                             = module.vpc_hub2.network_id
